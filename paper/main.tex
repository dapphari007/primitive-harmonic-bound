...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig5_two_row.png}
\caption{Left: the gain over the second MRRW bound, $M_2-\kappa$, for the whole-cube exponent $\kappa_H$ of \cite{OAI2026} (orange) and for the two-row exponent $\kappa_{2\mathrm{row}}$ of Theorem~\ref{thm:exponent} (red); the dotted line marks $\delta_1=0.2350$, above which the bound of \cite[Thm.~1.1]{OAI2026} equals $\kappa_H$. Below $\delta_1$ the two-row curve coincides with $M_2$ because the optimum sits at the middle level $u=\tfrac12$. Right: the improvement $\kappa_H-\kappa_{2\mathrm{row}}$ on a logarithmic scale, positive at every $\delta$, together with $\kappa_{\mathrm{bin}}-\kappa_{2\mathrm{row}}$ where it is positive.}\label{fig:exp}
\end{figure}
